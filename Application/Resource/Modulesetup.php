<?php
class Bf_Application_Resource_Modulesetup extends Zend_Application_Resource_ResourceAbstract
{
    ...
    protected function _loadOptions ($fullpath)
    {
        if (file_exists($fullpath)) {
            switch (substr(trim(strtolower($fullpath)), - 3)) {
                case 'ini':
                    $cfg = new Zend_Config_Ini($fullpath, 
